...
try {
    ...
    // ── Verify request belongs to employee and is CHECKED_OUT ─────────────
    $check = $conn->prepare("
        SELECT id, gate_pass_code, checked_out_at
    ...
    if (!$row) {
        ...
        echo json_encode([
            "success" => false,
            "message" => "Request not found or not in CHECKED_OUT status"
        ]);
        exit;
    }
    ...
    $stmt = $conn->prepare("
        ...
        WHERE id = ?
    ");
    $stmt->bind_param("sii", $checked_in_at, $employee_id, $id);
    $stmt->execute();
    $stmt->close();
