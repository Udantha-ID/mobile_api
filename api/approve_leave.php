...
try {
  ...
  // 1) Security check + read request details (lock the leave request row)
  $q = "
    SELECT
      lr.leave_request_id,
      lr.employee_id,
  ...
  // 2) Lock employee_leave_balances row and check remaining
  $q2 = "
  ...
  // If no balance row exists, create it from employee_yearly_leave_balance
  if ($res2->num_rows === 0) {
 
    // 2A) Get yearly entitlement for this employee + policy
    $qy = "
    ...
    // 2C) Re-lock it (FOR UPDATE) and continue
    $stmt2->execute();
    $res2 = $stmt2->get_result();
  }
